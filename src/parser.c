...
// mod quantize conv weight to -1/1
void quantize_conv_weight(float *weight, int n)
{
    int i;
    // float avg = 0.0;
    // float max = 0;
...
{
    fprintf(stderr, "\n");
    fprintf(stderr, "loading connected layer\n");
...
{
    fprintf(stderr, "\n");
    fprintf(stderr, "loading conv layer\n");
    FILE *weights_fp = NULL;
    FILE *out_fp = NULL;
    FILE *bnm_fp = NULL;
    ...
    {
        if (QUANTIZE)
            out_fp = fopen("quant_weight.txt", "a");
        else
            out_fp = fopen("yolov2_tiny_voc_weight.txt", "a");
    }

    if (PRINT_BNM)
        bnm_fp = fopen("batch_norm.txt", "a");

    if (WRITE_WEIGHT_FILE)
        weights_fp = fopen("genv2_tiny_voc.weights", "a");
    ...
    fprintf(stderr, "l.c %d, l.group %d, l.n %d, l.size: %d\n", l.c, l.groups, l.n, l.size);
    fprintf(stderr, "l.inputs %d, l.outputs %d \n", l.inputs, l.outputs);
    fread(l.biases, sizeof(float), l.n, fp);

    if (WRITE_WEIGHT_FILE)
        fwrite(l.biases, sizeof(float), l.n, weights_fp);
    ...
    if (l.batch_normalize && (!l.dontloadscales))
    {
        ...
        if (WRITE_WEIGHT_FILE){
            fwrite(l.scales, sizeof(float), l.n, weights_fp);
            fwrite(l.rolling_mean, sizeof(float), l.n, weights_fp);
            fwrite(l.rolling_variance, sizeof(float), l.n, weights_fp);
        }

        // mod print rolling mean and var and scale
        if (PRINT_BNM)
        {
            int i;
    ...
    if (WRITE_WEIGHT_FILE)
        fwrite(l.weights, sizeof(float), num, weights_fp);
    // if(l.c == 3) scal_cpu(num, 1./256, l.weights, 1);

    if (l.flipped)
    {
        fprintf(stderr, "l.flipped is true\n");
    ...
    // mod get quantized
    if (QUANTIZE)
        quantize_conv_weight(l.weights, num);
    // mod print weight
    if (PRINT_WEIGHT)
    {
        int k;
